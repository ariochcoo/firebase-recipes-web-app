...
function App() {
  ...
  async function handleAddRecipe(newRecipe) {
    try {
      ...
      alert(`successfuly created a recipe with an ID ${response.id}`);
    } catch (error) {
      alert(error.message);
    }
  }
  ...
  async function handleDeleteRecipe(recipeId) {
    const deleteConfirmation = window.confirm(
      "Are you sure  OK for Yes. Cancel for No."
    );
    ...
    if (deleteConfirmation) {
      try {
        ...
        setCurrentRecipe(null);
        window.scrollTo(0, 0);
        alert(`succesfully deleted a recipe with a ID=${recipeId}`);
      } catch (error) {
        alert(error.message);
        throw error;
      }
  ...
  // categoryleri sıralamak için kullanılan function
  function lookupCategoryLabel(categoryKey) {
    const categories = {
      breadsSandwichesAndPizza: "Breads , Sandwiches,Pizza",
      eggsAndBreakfast: "Eggs and Breakfast",
      dessertsAndBakedGoods: "Dessert and Baked",
      fishAndSeefoods: "Fish",
    };
        ...
        <div className="row  filters">
          <label className="recipe-label input-label">
            ...
            >
              ...
              <option value="dessertsAndBakedGoods">Dessert and Baked</option>
              <option value="fishAndSeefoods">Fish</option>
            </select>
          </label>
